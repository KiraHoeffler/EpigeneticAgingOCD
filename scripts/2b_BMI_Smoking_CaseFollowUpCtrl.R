...
# FILTER
samplesheet <- samplesheet[which((samplesheet$Diagnosis == "CTRL" | samplesheet$Time_point == "M3")), ]
...
#EXPORT
write_xlsx(samplesheet, path = "output/Tables/Samplesheet_CaseFollowUpCtrl.xlsx")
...
write_xlsx(BMI_sum, path = "output/Tables/BMI_CaseFollowUpCtrl.xlsx")
# Estimate   Std. Error    t value  Pr(>|t|)     Variable
# (Intercept)  -2.318709e-05 1.205748e-04 -0.1923046 0.8475550  (Intercept)
# DiagnosisOCD -6.724712e-05 1.544272e-04 -0.4354617 0.6633510 DiagnosisOCD
BMI_sum


#Smoking
model <- lmrob(Smokingresid ~ Diagnosis + Age + Sex)
Smoking_sum <- as.data.frame(summary(model)$coefficients)
Smoking_sum$Variable <- rownames(Smoking_sum)
write_xlsx(Smoking_sum, path = "output/Tables/Smoking_CaseFollowUpCtrl.xlsx")
Smoking_sum

# Estimate Std. Error    t value     Pr(>|t|)     Variable
# (Intercept)  -0.10015103 0.02572718 -3.8928099 1.077956e-04  (Intercept)
# DiagnosisOCD -0.05670888 0.02740459 -2.0693205 3.885348e-02 DiagnosisOCD
...
ggsave("output/Figures//BMIsmoking/BMI_CaseFollowUpCtrl.pdf", BMI_plot, units = "cm", width = 9, height = 10, dpi = 400)
save(BMI_plot, file = "output/Figures/BMIsmoking/BMI_CaseFollowUpCtrl.RData")
...
ggsave("output/Figures/BMIsmoking//Smoking_CaseFollowUpCtrl.pdf", Smoking_plot, units = "cm", width = 9, height = 10, dpi = 400)
save(Smoking_plot, file = "output/Figures/BMIsmoking//Smoking_CaseFollowUpCtrl.RData")
