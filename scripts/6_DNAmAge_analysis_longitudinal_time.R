...
for (j in c(1:length(clocks))){
  ...
  model <- rlmer(formula = mAge ~ Time_point + Age + Sex + (1|Resp_nr))
...
write_xlsx(results_TP, path = "output/Tables/Results_OCD_TimeOnly.xlsx")
...
ggsave("output/Figures/Results/Results_OCD_TPonly.pdf", TP_plot, units = "cm", width = 8, height = 8, dpi = 400)
ggsave("output/Figures/Results/Results_OCD_TPonly.svg", TP_plot, device = "svg", units = "cm", width = 8, height = 8, dpi = 400)
save(TP_plot, file = "output/Figures/Results/Results_OCD_TPonly.RData")
...
for (j in c(1:length(clocks))){
  ...
  model <- rlmer(formula = mAge ~ Time_point + Age + Sex + BMI + Smoking + (1|Resp_nr))
  sum <- summary(model)
...
write_xlsx(results_TP_BMIsmoking, path = "output/Tables/Results_OCD_TimeOnly_BMIsmoking.xlsx")
...
ggsave("output/Figures/Results/Results_OCD_TP_BMIsmoking.pdf", TP_plot_Smoking_BMI, units = "cm", width = 8, height = 8, dpi = 400)
ggsave("output/Figures/Results/Results_OCD_TP_BMIsmoking.svg", TP_plot_Smoking_BMI, device = "svg", units = "cm", width = 8, height = 8, dpi = 400)
save(TP_plot_Smoking_BMI, file = "output/Figures/Results/Results_OCD_TP_BMIsmoking.RData")
...
for (j in c(1:length(clocks))){
  ...
  model <- rlmer(formula = mAge ~ Time_point + Age + Sex + Smoking + (1|Resp_nr))
  sum <- summary(model)
...
write_xlsx(results_TP_SmokingOnly, path = "output/Tables/Results_OCD_TimeOnly_SmokingOnly.xlsx")
...
TP_plot_Smoking <- ggplot(results_TP_SmokingOnly, aes(x=Time_point, y=clock, fill=fill_color))+
  geom_tile(color = "black") +
  geom_text(aes(label = rounded_p), color = "black", size = 3) +
  scale_fill_identity() +
  th + th_transparent + 
...
ggsave("output/Figures/Results/Results_OCD_TP_SmokingOnly.pdf", TP_plot_Smoking, units = "cm", width = 8, height = 8, dpi = 400)
ggsave("output/Figures/Results/Results_OCD_TP_SmokingOnly.svg", TP_plot_Smoking, device = "svg", units = "cm", width = 8, height = 8, dpi = 400)
save(TP_plot_Smoking, file = "output/Figures/Results/Results_OCD_TP_SmokingOnly.RData")
...
for (j in c(1:length(clocks))){
  ...
  model <- rlmer(formula = mAge ~ Time_point + Age + Sex + BMI + (1|Resp_nr))
  sum <- summary(model)
...
write_xlsx(results_TP_BMIOnly, path = "output/Tables/Results_OCD_TimeOnly_BMIOnly.xlsx")
...
TP_plot_BMI <- ggplot(results_TP_BMIOnly, aes(x=Time_point, y=clock, fill=fill_color))+
  geom_tile(color = "black") +
  geom_text(aes(label = rounded_p), color = "black", size = 3) +
  scale_fill_identity() +
  th + th_transparent + 
...
ggsave("output/Figures/Results/Results_OCD_TP_BMIOnly.pdf", TP_plot_BMI, units = "cm", width = 8, height = 8, dpi = 400)
ggsave("output/Figures/Results/Results_OCD_TP_BMIOnly.svg", TP_plot_BMI, device = "svg", units = "cm", width = 8, height = 8, dpi = 400)
save(TP_plot_BMI, file = "output/Figures/Results/Results_OCD_TP_BMIOnly.RData")
...
for (j in c(1:length(clocks))){
  ...
  model <- rlmer(mAge ~ Time_point * comorb_data + Age + Sex + (1|Resp_nr))
  sum <- summary(model)
...
write_xlsx(results_TP_comorb_selected, path = "output/Tables/Results_OCD_TimeOnly_comorb.xlsx")
...
for (j in c(1:length(clocks))){
  ...
  for (k in 1:7){
    ...
    if(nrow(medic_data_table) > 1){
      ...
      model <- rlmer(mAge ~ Time_point * medic_data + Age + Sex + (1|Resp_nr))
      sum <- summary(model)
...
write_xlsx(results_TP_medic_selected, path = "output/Tables/Results_OCD_TimeOnly_medic.xlsx")
...
for (j in c(1:length(clocks))){
  ...
  pD4 <- star_p_value(results_TP[which(results_TP$clock == clock & results_TP$Time_point == "Post"), "adj_p"])
  pM3 <- star_p_value(results_TP[which(results_TP$clock == clock & results_TP$Time_point == "Follow-up"), "adj_p"])
  
  diff <- abs((max(df_avg$avg) + max(df_avg$se)) - (max(df_avg$avg) - max(df_avg$se)))
  
  Age_plot <- ggplot(df_avg, aes(x = Time_point, y = avg, group = 1)) +
    ...
    annotate("segment", x = 1, xend = 2, y = max(df_avg$avg) + max(df_avg$se) + 0.05*diff, 
             yend = max(df_avg$avg) + max(df_avg$se) +  0.05*diff, color = "black") +
    annotate("text", x = 1.5, y = max(df_avg$avg) + max(df_avg$se) +  0.14*diff, label = pD4, size = 5) +
    annotate("segment", x = 1, xend = 3, y = max(df_avg$avg) + max(df_avg$se) +  0.23*diff, 
             yend = max(df_avg$avg) + max(df_avg$se) +  0.23*diff, color = "black") +
    annotate("text", x = 2, y = max(df_avg$avg) + max(df_avg$se) +  0.28*diff, label = pM3, size = 5)
  Age_plot
  
  ggsave(paste0("output/Figures//TimeOnly/LongitudinalTimeOnly_AgeAccel_", clock, "_categ.pdf"), Age_plot, units = "cm", width = 9, height = 10, dpi = 400)
  save(Age_plot, file = paste0("output/Figures//TimeOnly/LongitudinalTimeOnly_AgeAccel_", clock, "_categ.RData"))
}
...
write_xlsx(results_TP_Sensitivity, path = "output/Tables/Sensitivity_OCD_TimeOnly.xlsx")
...
# adjust p values
write_xlsx(results_TP_Sensitivity_BMIsmoking, path = "output/Tables/Sensitivity_OCD_TimeOnly_BMIsmoking.xlsx")
